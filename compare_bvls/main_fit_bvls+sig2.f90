...
integer :: num2,tabsetf,Tnumf, zero = 0, summ = 0, debut, fin
...
!open(420, file='out.bvls', status='new', action='write')
open(420, file='out.bvls', status='old', action='read')
if(.true.) then
  !print*,'SOL BVLS-------------------------------------------'
  ...
  do j=1,ixt2
    ...
    read(420,*) tmp_out
    print*, solnr_sol(j), tmp_out, tmp_out - solnr_sol(j)
